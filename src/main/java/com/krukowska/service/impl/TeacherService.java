...
public class TeacherService implements ITeacherService {
    ...
    public TeacherDTO findTeacherByPesel(String pesel){
            Teacher teacher =  teacherRepository.findTeacherByPesel(pesel)
                     .orElseThrow(() -> new TeacherRequestException("No teacher under the pesel of: "+ pesel, HttpStatus.NOT_FOUND));
            return teacherConverter.toDTO(teacher);
    }
    @Override
    public List<TeacherDTO> findByAgeNGender(Gender gender, int age){
        ...
        if(teacher.isEmpty()) throw new TeacherRequestException("No" + gender + "teachers, that are " + age + "years old", HttpStatus.NOT_FOUND);
        else
